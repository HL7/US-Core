...
- **Menu Bar & Change Log**: To reduce menu clutter, consolidate topics, and improve change log navigation. We updated the main menu, moving [Changes Between Versions] and [Change Log] pages to a new [Version History] menu bar item. We also moved [Security] to the [Conformance] menu, and relocated [Examples] under [FHIR Artifacts]. We resorted the change log, resorting trackers based on their change impact: non-compatible, substantive, and non-substantive.
- **Changes Between Versions**: In anticipation of FHIR R6, the guidance in the Changes Between Versions section has been updated to address client and server expectations when transitioning between FHIR versions. Topics include:
  ...
  - [Data Preservation](changes-between-versions.html#expectation-that-data-is-preserved-bewteen-versions)
- **[ValueSet and CodeSystem Versions](terminology.html#us-core-valueset-and-codesystem-versions)**:  We document US Core's decision to publish **snapshots** of terminology at release time but **not** pin canonical references to specific versions.  We introduce *Value Set Packages* that tie US Core Versions to annual version updates of Value Sets and Code Systems as a resource for implementers who require them. This guidance aligns with the evolution of real-world terminology, reduces breaking changes, and supports implementers.