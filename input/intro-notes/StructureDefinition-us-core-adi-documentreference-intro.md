...
1. a status
2. a code describing the type of ADI document <span class="bg-success" markdown="1">(for example, POLST or Living Will)</span><!-- new-content -->
...
2. <span class="bg-success" markdown="1">an attester or legal attester</span><!-- new-content -->
3. <span class="bg-success" markdown="1">attestation date</span><!-- new-content -->
4. a code identifying the specific <span class="bg-success" markdown="1">implementation requirements that the document conforms to</span><!-- new-content -->  — over and above the content's MIME type
...
- This profile and the [US Core Observation ADI Documentation Profile] meet the [U.S. Core Data for Interoperability (USCDI)](https://www.healthit.gov/isp/united-states-core-data-interoperability-uscdi) *Advance Directive Observation* Data Element requirements.
- This profile can communicate the contents of a Plan of Medical Orders (PMO) document, such as POLST, MOLST, or other state-specific forms. PMO documents, typically stored as scanned images in EMRs/EHRs, follow the patient across care settings.
- <span class="bg-success" markdown="1">The [US Core ServiceRequest Profile] can be used to represent an order based on the PMO document.  MORE DOCUMENTATION HERE</span><!-- new-content -->
...
- The DocumentReference Attestor R5 Extension replaces the R4 `DocumentReference.authenticator` to allow multiple authenticators, including patient attestation. The examples illustrate how this extension is used to represent patient and provider attestations for various ADI documents.
