...
- For the current job, systems **SHALL** omit `observation.effectivePeriod.end` to indicate it is ongoing.<sup>[§][CONF-0432]</sup>
- When the industry is known, but the occupation is not, systems **SHALL** use the value "unknown" from the [DataAbsentReason Code System].<sup>[§][CONF-0433]</sup> However, when the occupation is known but the industry is not, systems **SHALL** omit the industry `Observation.component`.<sup>[§][CONF-0434]</sup> The guide provides examples for both scenarios.
