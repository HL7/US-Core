...
1. Contact information<sup>3</sup>
1. A communication endpoint<sup>3</sup>
...
- <sup>3</sup> A [Direct address] can be represented in the `telecom` element using the [US Core Direct email Extension] or in a referenced [Endpoint] as a "direct-project" endpoint connection type.
- <sup>2</sup>When selecting role codes
  - <span class="bg-success" markdown="1">Unless exchanging legacy or text-only data, medical specialty codes **SHOULD** be taken from NUCC or SNOMED CT.<sup>[§][CONF-0894]</sup></span><!-- new-content -->
